        ...
        <>   
            ...
            <div className={`${styles.otherPlayer}`} style={style}>
                ...
                <div className={`${styles.playerInfoContainer} ${gameState.turn === (index + meIndex) % numPlayers && index !== 0 && !gameState.handComplete && !flipping && !flopping ? styles.yellowHalo : ''}`} style={{borderRadius: basefont/2, display: burgerOpen ? 'none' : ''}} >
                    ...
                    <div className={styles.nameAndChips}>
                        ...
                        {gameState.handComplete && winAmount > 0 && <h1 className={styles.winAmount} style={{fontSize: basefont * 1.5}}>+${(winAmount / 100).toFixed(decimalAmount)}</h1>}
                    </div>
                    ...
                    <div className={styles.moneyInPot} style={{...chipStyle, borderRadius: basefont/2, visibility: !player.folded && !player.action  && !player.bet && !player.allIn ? 'hidden' : 'visible'}}>
                        ...
                        {player.allIn && 
                            ...
                            <h1 style={{fontSize: basefont, color: 'red'}}>{`All In $${gameState.flipping? (player.allIn / 100).toFixed(decimalAmount) : flipping? ((player.allIn + player.bet) / 100).toFixed(decimalAmount) : ((player.bet) / 100).toFixed(decimalAmount) }`}</h1>
                        }
                        ...
                        <h1 className={styles.otherBet} style={{fontSize: containerSize * .03}}>${(player.bet / 100).toFixed(decimalAmount)}</h1>
                        }  
            ...
            <div className={styles.me} style={{...style, display: burgerOpen ? 'none': 'flex'}} >
                ...
                <div className={`${styles.myPocket}`} style={{...cardStyle}} >
                    ...
                    { player.allIn && !gameState.handComplete &&<> <h1 className={styles.meAllIn} style={{fontSize: basefont * 2, color: 'red', borderRadius: containerSize * .02}}>{`All In $${gameState.flipping? (player.allIn / 100).toFixed(decimalAmount) : flipping? ((player.allIn + player.bet) / 100).toFixed(decimalAmount) : ((player.bet) / 100).toFixed(decimalAmount) }`}</h1><h1 className={styles.meMaxWin} style={{fontSize: containerSize * .03}}>{`Max Win: ${(player.maxWin / 100).toFixed(decimalAmount)}`}</h1></>}
                    </div>
                    ...
                    <div className={styles.meInfoContainer}>
                        ...
                        <div className={styles.MymoneyInPotContainer} style={{...chipStyle, borderRadius: containerSize * .3}}>
                            ...
                            {player.bet > 0 && 
                                <h1 className={styles.myMoneyInPot} style={{fontSize: containerSize * .03}}>${(player.bet / 100).toFixed(decimalAmount)}</h1>}
                        </div>
                        }
